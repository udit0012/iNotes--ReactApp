  ...
  return <div className='w-full h-full flex px-5 justify-center'>
    <form className="px-3 py-3 w-full" onSubmit={submitlogin}>
      <div className="mb-4">
        ...
        <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-green-600" id="email" name="email" type="text" onChange={(e)=>{setEmail(e.target.value)}} value={email} placeholder="john@gmail.com" />
      </div>
      <div className="mb-2">
        <label className="block text-green-600 text-sm font-bold mb-2" htmlFor="password">
          Password
        </label>
        <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-green-600" id="password" name="password" onChange={(e)=>{setPassword(e.target.value)}} value={password} type="password" placeholder="Password" />
      </div>
